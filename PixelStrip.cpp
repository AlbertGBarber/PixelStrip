...
PixelStrip::PixelStrip(uint16_t n, uint8_t p, uint8_t t)
    ...
    , bByteIndex(0)
    , rainbowOffset(0)
    , rainbowCycleRun(false)
    , rainbowOffsetDirct(1)
    , prevRainbowCycleTime(0) {};
// improvments TODO:
...
void PixelStrip::fillStrip(uint32_t color, boolean showNow) { fillStripSection(color, 0, numLEDs - 1, showNow); }
...
void PixelStrip::fillRainbow(boolean showNow) { fillRainbowSection(0, numLEDs - 1, showNow); }
...
// sets the offset of the wheel function
// this lets you adjust the start/end points of the rainbow
void PixelStrip::setRainbowOffset(uint8_t offset) { rainbowOffset = offset; }

// returns the current rainbow offset amount
uint8_t PixelStrip::getRainbowOffset() { return rainbowOffset; }

// sets the frequency and direction of change of the wheel rainbow cycle
// lets you constantly shift the rainbow while an effect is active
// offsetDirct = true will set the rainbow to move positivly along the strip, false will set to reverse
void PixelStrip::setRainbowOffsetCycle(uint32_t offsetFreq, boolean offsetDirct) {
    rainbowOffsetfreq = offsetFreq;
    if (offsetDirct) {
        rainbowOffsetDirct = 1;
    } else {
        rainbowOffsetDirct = -1;
...
// turns the wheel rainbow cycle on/off (true to turn on)
void PixelStrip::runRainbowOffsetCycle(boolean cycleRun) { rainbowCycleRun = cycleRun; }
...
uint32_t PixelStrip::Wheel(byte WheelPos) {
    if (rainbowCycleRun) {
        ...
        if (currentTime - prevRainbowCycleTime > rainbowOffsetfreq) {
            prevRainbowCycleTime = currentTime;
            rainbowOffset = (uint16_t)(255 + rainbowOffset + rainbowOffsetDirct) % 255;
        }
    }
    WheelPos = (uint16_t)(255 - WheelPos + rainbowOffset) % 255;
    yield();
    if (WheelPos < 85) {
...
uint32_t PixelStrip::randColor() { return Color(random(255), random(255), random(255)); }
...
void PixelStrip::dimPixel(int pixel) { setPixelColor(pixel, dimColor(getPixelColor(pixel), 1)); }
...
// crossfades individual pixels through a series of colors in gradLength number of steps, using pallet for colors
// each pixel is given a random offset that will place it somewhere in the cycle of colors
// so you get a mix of colors from pallet that gradually fade between each other
// for different looks you can use grouping, which will give groups of leds the same offset
//(the group size is randomly assigned up to the grouping value)
// Two possible function modes:
//      Mode 0: As described above, all leds have a random offset
//      Mode 1: The offset for all the leds is constrained to gradLength, this means they will all transition to the next color at similar times
//      (for mode 1, grouping doesn't matter)
void PixelStrip::shiftingSea(uint32_t pallet[], uint8_t palletLength, uint8_t gradLength, uint8_t mode, uint8_t grouping, int numCycles, int wait) {
    uint16_t pixelOffsets[numLEDs]; // storage array for led offset

    // crossfade function requires RGB colors,
    // so we make an RGB version of the pallet
    ...
    if (mode == 0){
        ...
        for (int i = 0; i < numLEDs; i++) {
            pixelOffsets[i] = random(gradLength);
        }
    }

    // variables for the main loop
    uint16_t step, offset;
    uint8_t currentColorIndex, gradStep;
    RGB currentColor, nextColor;
    uint32_t color;

    // for numCycles, fade each led to the next color on step at a time, accounting for each offset
    // and wrapping as needed
    for (int i = 0; i < numCycles; i++) {
        for (int j = 0; j < numLEDs; j++) {
            offset = pixelOffsets[j]; // get the offset for the current led
            step = (i + offset) % totalCycleLength; // where we are in the cycle of all the colors
            gradStep = (i + offset) % gradLength; // what step we're on between the current and next color
            currentColorIndex = step / gradLength; // what color we've started from (integers always round down)
            currentColor = palletRGB[currentColorIndex];
            nextColor = palletRGB[(currentColorIndex + 1) % palletLength]; // the next color, wrapping to the start of the pallet as needed
            color = getCrossFadeStep(currentColor, nextColor, gradLength, gradStep);
            setPixelColor(j, color);
        }
        show();
        delay(wait);
...
void PixelStrip::genShiftingSeaGroupedOffsetArray(uint16_t pixelOffsets[], uint8_t grouping, uint16_t totalCycleLength) {
    ...
    for (int i = 0; i < numLEDs;) {
        ...
        for (int j = 0; j < groupSize; j++) {
            ...
            pixelOffsets[i + j] = pixelOffsets[i];
        }
...
// false moves it backward
void PixelStrip::colorWipeGradient(int wait, uint16_t wipeLength, boolean forward, boolean alternate, boolean simult) { colorWipePallet(5, simult, 0, 0, 0, wipeLength, wait, forward, alternate); }
...
int16_t PixelStrip::patternSweepGetTrailLed(int8_t directionArray[2], uint16_t modAmmount, int16_t ledLocation, int8_t distance) { return (int16_t(ledLocation) + modAmmount + (directionArray[0] * distance) + directionArray[1]) % modAmmount; }
...
void PixelStrip::fireworksRand(uint8_t numRandLeds, int32_t preferedColor, int32_t BgColor, uint8_t dimFactor, uint8_t colorMode, int wait, int numCycles) { patternSweepRand(numRandLeds, preferedColor, BgColor, 5, 3, false, dimFactor, colorMode, wait, numCycles); }
...
void PixelStrip::fireworksSet(uint8_t numRandLeds, uint8_t dimFactor, uint32_t pallet[], uint8_t palletLength, int32_t BgColor, uint8_t colorMode, int wait, int numCycles) {
    ...
    if (palletLength == 0) {
        dimmedPalletLength = numRandLeds;
    } // if we're doing random colors, we need to set the pallet length correctly
    ...
    for (int i = 0; i <= numCycles; i++) {
        ...
        for (int j = 0; j < numSimultWaves; j++) {
            for (int k = 0; k < totalFadeLength; k++) {
                ...
                if (k > 0) {
                    segColor = dimColor(segColor, 1);
                }; // real quick, dirty way to make the trails dim more dramatically, replace this with HSV dimming later
...
int16_t PixelStrip::getPixelNumFromLineNum(SegmentSet segmentSet, uint16_t maxSegLength, uint8_t segNum, byte lineNum) { return getSegmentPixel(segmentSet, segNum, (uint16_t(lineNum) * uint16_t(segmentSet.getTotalSegLength(segNum)) / maxSegLength)); }
...
// note not compatiable with rainbow/gradients, this is due to using getCrossFadeStep for dimming
...
void PixelStrip::sonarSpin(SegmentSet segmentSet, uint32_t pallet[], byte palletLength, byte pattern[], uint8_t patternLength, uint32_t BgColor, uint8_t totalFadeLength, uint8_t spacing, boolean direct, boolean alternate, boolean bounce, boolean bounceChange, int numCycles, uint8_t wait) {
    ...
    uint8_t nextPalletIndex;
    int8_t bounceFade[numSimultWaves][2]; // placeholder for next color for a wave
    ...
    for (int i = 0; i <= numCycles; i++) {
        ...
        for (int j = 0; j < numSimultWaves; j++) {
            // if we're bouncing, then the trails are not touched after a bounce b/c the wave changes direction
            // this fills in the remaining trail with the BG
            if (bounceFade[j][0] > 0) {
                if (activeWavePos[j][2] == -1) {
                    drawSegLineSimple(segmentSet, maxSegLength - 1 - bounceFade[j][0], BgColor, 1);
                } else {
                    drawSegLineSimple(segmentSet, bounceFade[j][0], BgColor, 1);
                }
                bounceFade[j][0]--;
            }
            ...
            for (int k = 0; k < totalFadeLength; k++) {
                ...
                if (k > 0) {
                    lineColor = dimColor(lineColor, 1);
                }; // real quick, dirty way to make the trails dim more dramatically, replace this with HSV dimming later
                drawSegLineSimple(segmentSet, wavePos, lineColor, 1);
            }
            bounceFade[j][1] = nextPalletIndex;
            nextPalletIndex = (activeWavePos[j][0] + numSimultWaves) % palletLength; // get the next color ready (skip forward by numSimultWaves, so stop repeating colors)
            activeWavePos[j][1] = activeWavePos[j][1] + activeWavePos[j][2]; // get the next wave head position
            // if bounce, check to change direction / color, otherwise check for color change
            if (bounce) {
                ...
                if (activeWavePos[j][1] > maxSegLength - 1 && activeWavePos[j][2] == 1) {
                    bounceFade[j][0] = totalFadeLength;
                    activeWavePos[j][2] = -1;
                    // if the wave started at then last segment, change it's color
                    if (startDirect[j] == -1 || bounceChange) {
                        activeWavePos[j][0] = nextPalletIndex;
